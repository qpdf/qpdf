...
{
    test_to_number(str, wanted, error, QUtil_12::string_to_int);
}

void
test_to_ll(char const* str, long long wanted, bool error)
{
    test_to_number(str, wanted, error, QUtil_12::string_to_ll);
}

void
test_to_uint(char const* str, unsigned int wanted, bool error)
{
    test_to_number(str, wanted, error, QUtil_12::string_to_uint);
}

void
test_to_ull(char const* str, unsigned long long wanted, bool error)
{
    test_to_number(str, wanted, error, QUtil_12::string_to_ull);
}
...
string_conversion_test()
{
    ...
    // a non-C locale.
    set_locale();
    std::cout << QUtil_12::int_to_string(16059) << std::endl
              << QUtil_12::int_to_string(16059, 7) << std::endl
              << QUtil_12::int_to_string(16059, -7) << std::endl
              << QUtil_12::double_to_string(3.14159, 0, false) << std::endl
              << QUtil_12::double_to_string(3.14159, 3) << std::endl
              << QUtil_12::double_to_string(1000.123, -1024, false) << std::endl
              << QUtil_12::double_to_string(.1234, 5, false) << std::endl
              << QUtil_12::double_to_string(.0001234, 5) << std::endl
              << QUtil_12::double_to_string(.123456, 5) << std::endl
              << QUtil_12::double_to_string(.000123456, 5) << std::endl
              << QUtil_12::double_to_string(1.01020, 5, true) << std::endl
              << QUtil_12::double_to_string(1.00000, 5, true) << std::endl
              << QUtil_12::double_to_string(1, 5, true) << std::endl
              << QUtil_12::double_to_string(1, 5, false) << std::endl
              << QUtil_12::double_to_string(10, 2, false) << std::endl
              << QUtil_12::double_to_string(10, 2, true) << std::endl
              << QUtil_12::int_to_string_base(16059, 10) << std::endl
              << QUtil_12::int_to_string_base(16059, 8) << std::endl
              << QUtil_12::int_to_string_base(16059, 16) << std::endl
              << QUtil_12::int_to_string_base(5000093552LL, 10) << std::endl;
    ...
    char* tmp = QUtil_12::copy_string(embedded_null);
    ...
    auto tmp2 = QUtil_12::make_shared_cstr(embedded_null);
    if (memcmp(tmp2.get(), embedded_null.c_str(), 7) == 0) {
        std::cout << "compare okay" << std::endl;
    } else {
        std::cout << "compare failed" << std::endl;
    }
    auto tmp3 = QUtil_12::make_unique_cstr(embedded_null);
    ...
    std::string int_max_str = QUtil_12::int_to_string(INT_MAX);
    std::string int_min_str = QUtil_12::int_to_string(INT_MIN);
    long long int_max_plus_1 = static_cast<long long>(INT_MAX) + 1;
    long long int_min_minus_1 = static_cast<long long>(INT_MIN) - 1;
    std::string int_max_plus_1_str = QUtil_12::int_to_string(int_max_plus_1);
    std::string int_min_minus_1_str = QUtil_12::int_to_string(int_min_minus_1);
    std::string small_positive = QUtil_12::uint_to_string(16059U);
    std::string small_negative = QUtil_12::int_to_string(-16059);
...
fopen_wrapper_test()
{
    try {
        ...
        FILE* f = QUtil_12::safe_fopen("/this/file/does/not/exist", "r");
    ...
    assert(QUtil_12::file_can_be_opened("qutil.out"));
    assert(!QUtil_12::file_can_be_opened("/does/not/exist"));
}
...
{
    std::string val;
    std::cout << "IN_TESTSUITE: " << QUtil_12::get_env("IN_TESTSUITE", &val)
              << ": " << val << std::endl;
    // Hopefully this environment variable is not defined.
    std::cout << "HAGOOGAMAGOOGLE: " << QUtil_12::get_env("HAGOOGAMAGOOGLE")
              << std::endl;
}
...
{
    std::string result = QUtil_12::toUTF8(val);
    std::cout << "0x" << QUtil_12::uint_to_string_base(val, 16) << " ->";
    if (val < 0xfffe) {
        std::cout << " " << result;
    } else {
    ...
    // Boundary conditions for QUtil_12::get_next_utf8_codepoint, which is
    // also tested indirectly through test_pdf_unicode.cc.
    std::string utf8 = "\xcf\x80\xcf\x30\xEF\xBF\x30\x31\xcf";
    size_t pos = 0;
    bool error = false;
    assert(QUtil_12::get_next_utf8_codepoint(utf8, pos, error) == 0x3c0);
    assert(pos == 2);
    assert(!error);
    assert(QUtil_12::get_next_utf8_codepoint(utf8, pos, error) == 0xfffd);
    assert(pos == 3);
    assert(error);
    assert(QUtil_12::get_next_utf8_codepoint(utf8, pos, error) == 0x30);
    assert(pos == 4);
    assert(!error);
    assert(QUtil_12::get_next_utf8_codepoint(utf8, pos, error) == 0xfffd);
    assert(pos == 6);
    ...
    assert(pos == 7);
    assert(!error);
    assert(QUtil_12::get_next_utf8_codepoint(utf8, pos, error) == 0x31);
    assert(pos == 8);
    assert(!error);
    assert(QUtil_12::get_next_utf8_codepoint(utf8, pos, error) == 0xfffd);
    assert(pos == 9);
...
{
    std::string result = QUtil_12::toUTF16(val);
    std::cout << "0x" << QUtil_12::uint_to_string_base(val, 16) << " ->";
    for (auto const& ch: result) {
        std::cout << " "
                  << QUtil_12::int_to_string_base(
...
to_utf16_test()
{
    ...
    std::string s(QUtil_12::utf8_to_utf16("\xcf\x80"));
    std::cout << QUtil_12::utf16_to_utf8(s) << std::endl;
    std::cout << QUtil_12::utf16_to_utf8(s + ".") << std::endl;
    std::cout << "LE: " << QUtil_12::utf16_to_utf8("\xff\xfe\xc0\x03")
              << std::endl;
}
...
utf8_to_ascii_test()
{
    ...
    std::cout << input << std::endl
              << QUtil_12::utf8_to_ascii(input) << std::endl
              << QUtil_12::utf8_to_ascii(input, '*') << std::endl;
    std::string a = QUtil_12::utf8_to_win_ansi(input, '*');
    std::string b = QUtil_12::utf8_to_mac_roman(input, '*');
    std::cout << "<"
              << QUtil_12::int_to_string_base(
...
check_analyze(std::string const& str, bool has8bit, bool utf8, bool utf16)
{
    ...
    bool is_utf16 = false;
    QUtil_12::analyze_encoding(str, has_8bit_chars, is_valid_utf8, is_utf16);
...
print_alternatives(std::string const& str)
{
    std::vector<std::string> result =
        QUtil_12::possible_repaired_encodings(str);
    size_t n = result.size();
    for (size_t i = 0; i < n; ++i) {
        std::cout << i << ": " << QUtil_12::hex_encode(result.at(i))
                  << std::endl;
    }
}
...
transcoding_test()
{
    ...
    transcoding_test(
        &QUtil_12::pdf_doc_to_utf8, &QUtil_12::utf8_to_pdf_doc, 24, 31, "?");
    std::cout << "bidirectional pdf doc low done" << std::endl;
    transcoding_test(
        &QUtil_12::win_ansi_to_utf8,
    ...
    std::string output;
    assert(!QUtil_12::utf8_to_ascii(input1, output));
    assert(!QUtil_12::utf8_to_ascii(input2, output));
    assert(QUtil_12::utf8_to_ascii(input3, output));
    assert(QUtil_12::utf8_to_win_ansi(input1, output));
    assert(!QUtil_12::utf8_to_win_ansi(input2, output));
    assert(QUtil_12::utf8_to_win_ansi(input3, output));
    assert(QUtil_12::utf8_to_mac_roman(input1, output));
    assert(!QUtil_12::utf8_to_mac_roman(input2, output));
    assert(QUtil_12::utf8_to_mac_roman(input3, output));
    assert(QUtil_12::utf8_to_pdf_doc(input1, output));
    assert(!QUtil_12::utf8_to_pdf_doc(input2, output));
    assert(QUtil_12::utf8_to_pdf_doc(input3, output));
    ...
    std::string utf8 = QUtil_12::pdf_doc_to_utf8(pdfdoc);
    ...
    std::cout << QUtil_12::pdf_doc_to_utf8(other_doc) << std::endl;
    std::string other_utf8 =
        other + QUtil_12::toUTF8(0x9f) + "w" + QUtil_12::toUTF8(0xad) + "w";
    std::string other_to_utf8;
    assert(!QUtil_12::utf8_to_pdf_doc(other_utf8, other_to_utf8));
    ...
    assert(!QUtil_12::utf8_to_pdf_doc(fe_ff, pdfdoc));
    assert(pdfdoc == "?\xfe\xff potato");
    assert(!QUtil_12::utf8_to_pdf_doc(ff_fe, pdfdoc));
    assert(pdfdoc == "?\xff\xfe potato");
    assert(!QUtil_12::utf8_to_pdf_doc(ef_bb_bf, pdfdoc));
    assert(pdfdoc == "?\xef\xbb\xbf potato");
    assert(QUtil_12::utf8_to_pdf_doc("\xc3\xbe\xc3\xbe", pdfdoc));
    assert(QUtil_12::utf8_to_pdf_doc("\xc3\xaf\xc2\xbb\xc2\xbe", pdfdoc));
}

void
print_whoami(char const* str)
{
    auto dup = QUtil_12::make_unique_cstr(str);
    std::cout << QUtil_12::getWhoami(dup.get()) << std::endl;
}
...
{
    bool actual = QUtil_12::same_file(file1, file2);
...
{
    try {
        fclose(QUtil_12::safe_fopen("qutil.out", "r"));
        fclose(QUtil_12::safe_fopen("other-file", "r"));
...
path_test()
{
    auto check = [](bool print, std::string const& a, std::string const& b) {
        auto result = QUtil_12::path_basename(a);
        if (print) {
...
read_from_file_test()
{
    std::list<std::string> lines = QUtil_12::read_lines_from_file("other-file");
    ...
    // Test the other versions and make sure we get the same results
    {
        ...
        assert(QUtil_12::read_lines_from_file(infs) == lines);
        FILE* fp = QUtil_12::safe_fopen("other-file", "rb");
        assert(QUtil_12::read_lines_from_file(fp) == lines);
        fclose(fp);
    }

    // Test with EOL preservation
    std::list<std::string> lines2 =
        QUtil_12::read_lines_from_file("other-file", true);
    ...
    {
        std::ifstream infs("other-file", std::ios_base::binary);
        assert(QUtil_12::read_lines_from_file(infs, true) == lines2);
        FILE* fp = QUtil_12::safe_fopen("other-file", "rb");
        assert(QUtil_12::read_lines_from_file(fp, true) == lines2);
        fclose(fp);
    }

    std::shared_ptr<char> buf;
    size_t size = 0;
    QUtil_12::read_file_into_memory("other-file", buf, size);
    ...
    Pl_Buffer b2("buffer");
    // QUtil_12::file_provider also exercises QUtil_12::pipe_file
    QUtil_12::file_provider("other-file")(&b2);
...
{
    std::string actual = QUtil_12::hex_encode(input);
    if (expected != actual) {
...
{
    std::string actual = QUtil_12::hex_decode(input);
    if (expected != actual) {
        std::cout << "hex encode " << input << ": expected = " << expected
                  << "; actual = " << actual << std::endl;
    }
...
{
    try {
        fclose(QUtil_12::safe_fopen(filename, "r"));
        assert(false);
...
rename_delete_test()
{
    ...
    try {
        QUtil_12::remove_file("old\xcf\x80");
    } catch (QPDFSystemError&) {
    }
    assert_no_file("old\xcf\x80");
    std::cout << "create file" << std::endl;
    ;
    FILE* f1 = QUtil_12::safe_fopen("old\xcf\x80", "w");
    fprintf(f1, "one");
    fclose(f1);
    QUtil_12::read_file_into_memory("old\xcf\x80", buf, size);
    assert(memcmp(buf.get(), "one", 3) == 0);
    std::cout << "rename file" << std::endl;
    ;
    QUtil_12::rename_file("old\xcf\x80", "old\xcf\x80.~tmp");
    QUtil_12::read_file_into_memory("old\xcf\x80.~tmp", buf, size);
    assert(memcmp(buf.get(), "one", 3) == 0);
    assert_no_file("old\xcf\x80");
    std::cout << "create file" << std::endl;
    ;
    f1 = QUtil_12::safe_fopen("old\xcf\x80", "w");
    fprintf(f1, "two");
    fclose(f1);
    ...
    ;
    QUtil_12::remove_file("old\xcf\x80.~tmp");
    assert_no_file("old\xcf\x80");
...
timestamp_test()
{
    auto check = [](QUtil_12::QPDFTime const& t) {
        std::string pdf = QUtil_12::qpdf_time_to_pdf_time(t);
        std::string iso8601 = QUtil_12::qpdf_time_to_iso8601(t);
        std::cout << pdf << std::endl << iso8601 << std::endl;
        QUtil_12::QPDFTime t2;
        std::string iso8601_2;
        assert(QUtil_12::pdf_time_to_qpdf_time(pdf, &t2));
        assert(QUtil_12::qpdf_time_to_pdf_time(t2) == pdf);
        assert(QUtil_12::pdf_time_to_iso8601(pdf, iso8601_2));
        assert(iso8601 == iso8601_2);
    };
    check(QUtil_12::QPDFTime(2021, 2, 9, 14, 49, 25, 300));
    check(QUtil_12::QPDFTime(2021, 2, 10, 1, 19, 25, -330));
    check(QUtil_12::QPDFTime(2021, 2, 9, 19, 19, 25, 0));
    assert(!QUtil_12::pdf_time_to_qpdf_time("potato"));
    assert(QUtil_12::pdf_time_to_qpdf_time("D:20210211064743Z"));
    assert(QUtil_12::pdf_time_to_qpdf_time("D:20210211064743-05'00'"));
    assert(QUtil_12::pdf_time_to_qpdf_time("D:20210211064743+05'30'"));
    assert(QUtil_12::pdf_time_to_qpdf_time("D:20210211064743"));
    ...
    assert(QUtil_12::pdf_time_to_qpdf_time(
        QUtil_12::qpdf_time_to_pdf_time(QUtil_12::get_current_qpdf_time())));
}
...
memory_usage_test()
{
    auto u1 = QUtil_12::get_max_memory_usage();
    if (u1 > 0) {
        auto x = QUtil_12::make_shared_array<int>(10 << 20);
        auto u2 = QUtil_12::get_max_memory_usage();
        assert(u2 > u1);
    }
