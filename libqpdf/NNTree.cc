...
NNTreeIterator::updateIValue(bool allow_invalid)
{
    ...
    if ((item_number >= 0) && this->node.isDictionary()) {
        ...
        if (this->item_number + 1 < items.size()) {
            okay = true;
            this->ivalue.first = items.at(this->item_number);
            this->ivalue.second = items.at(1 + this->item_number);
        } else {
...
NNTreeIterator::getNextKid(PathElement& pe, bool backward)
{
    ...
    while (!found) {
        ...
        if ((pe.kid_number >= 0) && (pe.kid_number < kids.size())) {
            result = kids.at(pe.kid_number);
...
NNTreeIterator::increment(bool backward)
{
    ...
    while (valid() && (!found_valid_key)) {
        ...
        auto items = this->node.getKey(impl.details.itemsKey());
        if ((this->item_number < 0) || (this->item_number >= items.size())) {
            bool found = false;
        ...
        if (this->item_number >= 0) {
            ...
            } else if (!impl.details.keyValid(items.at(this->item_number))) {
    ...
    QPDFObjectHandle node, std::list<PathElement>::iterator parent)
{
    ...
    while (!done) {
        ...
        int nkids = kids.isArray() ? kids.size() : 0;
        auto items = node.getKey(impl.details.itemsKey());
        int nitems = items.isArray() ? items.size() : 0;
        ...
        if (nitems >= 2) {
            first = items.at(0);
            last = items.at((nitems - 1) & ~1);
        } else if (nkids > 0) {
            auto first_kid = kids.at(0);
            auto last_kid = kids.at(nkids - 1);
            if (first_kid.isDictionary() && last_kid.isDictionary()) {
                ...
                if ((first_limits.size() >= 2) && (last_limits.size() >= 2)) {
                    first = first_limits.at(0);
                    last = last_limits.at(1);
                }
            }
        }
        if (first.isInitialized() && last.isInitialized()) {
            ...
            if ((olimits.size() == 2)) {
                auto ofirst = olimits.at(0);
                auto olast = olimits.at(1);
    ...
    QPDFObjectHandle to_split, std::list<PathElement>::iterator parent)
{
    ...
    int start_idx = ((n / 2) & ~1);
    while (first_half.size() > start_idx) {
        second_half.appendItem(first_half.at(start_idx));
...
NNTreeIterator::insertAfter(QPDFObjectHandle key, QPDFObjectHandle value)
{
    ...
    if (items.size() < this->item_number + 2) {
    ...
    while (!done) {
        ...
        if (nkids > 0) {
            ...
            if (element->kid_number == nkids) {
                ...
                setItemNumber(QPDFObjectHandle(), -1);
                --element->kid_number;
                deepen(kids.at(element->kid_number), false, true);
                if (valid()) {
                    increment(false);
                    if (!valid()) {
                ...
                deepen(kids.at(element->kid_number), true, true);
            }
            done = true;
...
NNTreeImpl::withinLimits(QPDFObjectHandle key, QPDFObjectHandle node)
{
    ...
    if ((limits.size() >= 2) && details.keyValid(limits.at(0)) &&
        details.keyValid(limits.at(1))) {
        if (details.compareKeys(key, limits.at(0)) < 0) {
            result = -1;
        } else if (details.compareKeys(key, limits.at(1)) > 0) {
            result = 1;
        }
    } else {
...
{
    if (!(((items.size() > (2 * idx)) &&
           details.keyValid(items.at(2 * idx))))) {
    ...
    return details.compareKeys(key, items.at(2 * idx));
}
