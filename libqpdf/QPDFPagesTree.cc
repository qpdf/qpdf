...
QPDFPagesTree::iterator::updateIValue(bool allow_invalid)
{
    ...
    else
    {
        ...
        this->m->ivalue =
            cur.node.getKey("/Kids").getArrayItem(cur.kid_number);
    }
}
...
void
QPDFPagesTree::iterator::split(QPDFObjectHandle to_split,
                               std::list<PathElement>::iterator parent)
{
    ...
    if (is_root)
    {
        ...
        auto first_node = this->m->qpdf.makeIndirectObject(
    ...
    QPDFObjectHandle second_node = this->m->qpdf.makeIndirectObject(
        QPDFObjectHandle::newDictionary());
