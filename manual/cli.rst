...
.. qpdf:option:: --deduplicate-xobjects

   .. help: merge identical XObject streams into a single object

      If the input file contains distinct XObject stream objects that
      have identical data and dictionary entries, this option causes
      qpdf to merge them into a single stream object referenced
      multiple times.

   This option inspects every XObject stream in the file. If two
   streams are identical, it merges them so that they share a single
   underlying PDF object. (There are currently exceptions to this: no
   merges are performed in encrypted PDFs, and streams with a /Group
   key are not merged.) This can significantly reduce file size for
   PDFs generated by concatenating files or tools that redundantly
   embed resources as XObjects. This option is slower than standard
   processing.
...
- ``--deduplicate-xobjects``: remove redundant duplicate XObject
  stream data (see :qpdf:ref:`--deduplicate-xobjects`)
